...
- [Deploy to Google App Engine](#deploy-to-now)
...
Routing in Next.js is based on the file system, so `./pages/index.tsx` maps to the `/` route and
`./pages/about.tsx` would map to `/about`.

The `./public/static` directory maps to `/static` in the `next` server, so you can put all your
...
- Static file serving. `./public/static/` is mapped to `/static/`
...
1.  Install the `now` command-line tool either via the recommended [desktop tool](https://zeit.co/download) or via node with `yarn global add now`.
