...
export function handle(...args: any[]) {
  ...
  const errCode: HttpStatus | undefined = isOption(option)
    ? option.error
    : undefined;
  const successCode: HttpStatus | undefined = isOption(option)
    ? option.success
    : undefined;
  const usePaginate: boolean | undefined = isOption(option)
    ? option.usePaginate
    : undefined;
  return buildHttpDecorator({
    errCode,
